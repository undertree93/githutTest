...
var EventActions = {
    'Environment': {
       'Trend': function(i, propObj) { 
       		if (propObj.yrMarketRate != null){
				housingMarketRate = parseFloat(interestObj.yrMarketRate) / 52 / 100;
				alert (housingMarketRate);
			}
       	},
       'SalaryResidue': function(i, propObj) { /* ... */ },
       'OneTimeCash': function(i, propObj) { /* ... */ }
    },
    'House': {
        'Mortagage': function(i, propObj) { /* ... */ },
        'LumpsumPay': function(i, propObj) { /* ... */ },
        'BuyHouse': function(i, propObj) { /* ... */ },
        'SellHouse': function(i, propObj) { /* ... */ },
        'RentHouse': function(i, propObj) { /* ... */ }
    }
};
function buildGraph() {
	var eventHistory = [];
	...
	var latestEvent = 0;
	 ...
	 for (building in buildOrder) {
		...
		var buildingVar = {wkIncome:0, wkCost:0, wkInterest:0, wkMortgage:0, cash:0, debt:0, asset:0};
		buildVariable.push(buildingVar); 
	...
	*/
	...
	for (var wk = 0; wk < 520; wk++) {
		for (var j = 0; j < eventHistory.length; j++) {
			var evt = eventHistory[j];
			if (evt.eventStart == wk && evt.props !=null) {
				var propObj = 	eval('(props={' + evt.props + '})');
